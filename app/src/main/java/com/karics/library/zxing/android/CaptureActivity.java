...
public final class CaptureActivity extends Activity implements
	...
	public void handleDecode(Result rawResult, Bitmap barcode, float scaleFactor) {
		...
		if (fromLiveScan) {
			...
			intent.putExtra("codedrawbytes",rawResult.getRawBytes());
			Log.w(TAG, "rawResult text = " + rawResult.getText());

			String str3="";
			List localList;
			localList = (List) rawResult.getResultMetadata().get(ResultMetadataType.BYTE_SEGMENTS);
			if (localList == null) {
			}
			else {
				...
				while (localIterator.hasNext()) {
					...
					Log.w(TAG, "byte fragment = " + ByteUtils.bytesToStringNoSpace(arrayOfByte));
					//str3 = str3 + ByteUtils.bytesToStringNoSpace(arrayOfByte);
					str3 = str3 + ByteUtils.bytesToHexString(arrayOfByte);
				}
			}
			intent.putExtra("raw", str3);
			Log.w(TAG, "raw = " + str3);
